import {PropsIcon} from "@/types/public-types";

const SearchIcon = ({className}:PropsIcon) => {
    return (
        <svg width="32" height="32" className={`${className}`}  viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="M15.3333 28C22.3289 28 28 22.3289 28 15.3333C28 8.33773 22.3289 2.66667 15.3333 2.66667C8.33773 2.66667 2.66667 8.33773 2.66667 15.3333C2.66667 22.3289 8.33773 28 15.3333 28Z"
                stroke="#141414" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M29.3333 29.3333L26.6667 26.6667" stroke="#141414" strokeWidth="2" strokeLinecap="round"
                  strokeLinejoin="round"/>
        </svg>

    )
}
export default SearchIcon